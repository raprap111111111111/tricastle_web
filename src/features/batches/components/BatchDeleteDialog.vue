      ...
      <div class="bg-white rounded-xl px-4 py-3 border border-appleCore-200">
        <div class="flex items-center gap-2">
          <span class="font-mono text-xs text-apricot-600 font-semibold bg-apricot-50 px-2 py-0.5 rounded">
            Batch #{{ props.batch?.batch_number }}
          </span>
          <span
            v-if="props.batch?.is_active"
        ...
        <p class="text-blueberry-800 font-semibold text-sm mt-1.5">
          {{ props.batch?.name }}
        </p>
        <p v-if="props.batch?.country" class="text-blueberry-500 text-xs mt-0.5">
