...
onMounted(async () => {
  ...
  // 👇 Pre-populate filters from URL query params (came from Final List page)
  if (route.query.search) {
    searchQuery.value = String(route.query.search)
  }
  if (route.query.batchId) {
    selectedBatchId.value = Number(route.query.batchId)
  }
            ...
            <Select v-model="selectedBatchId" :options="batchOptions" option-label="label" option-value="value"
              ...
              <template #option="{ option }">
                <div class="flex items-center gap-2 w-full">
                  ...
                  <span v-if="option.isActive" class="text-[10px] px-1.5 py-0.5 rounded-full
