  ...
  <div class="flex flex-col gap-6 p-8 max-w-[1400px] mx-auto">
    ...
    <template v-else>
      ...
      <header class="flex items-start justify-between gap-4">
        ...
        <!-- ✅ Export Button -->
        <Button label="Export Excel" icon="pi pi-file-excel" severity="secondary" outlined @click="openExportDialog" />
      </header>
      ...
      <div class="flex flex-col gap-3 p-4 bg-white border border-appleCore-100 rounded-xl">
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="min-w-[240px]">
            <Select v-model="selectedBatchId" :options="batchOptions" optionLabel="label" optionValue="value"
              placeholder="Select Batch" class="w-48 text-xs" filter filterPlaceholder="Type batch name or #..."
              showClear>
      ...
      <AppCard :padding="'none'" :shadow="'soft'">
        <ApplicantTable :applicants="filteredApplicants" :pagination="null" :loading="store.loading" :submitting="false"
          @page-change="() => { }" @limit-change="() => { }" @delete="() => { }" />
      </AppCard>
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  <Select v-model="city" :options="cityOptions" option-label="label" option-value="value" :placeholder="!province
                    ...
                    : loadingCities
                      ? 'Loading cities...'
                      : 'Select city or municipality...'" class="w-full" size="small" filter show-clear
                    :loading="loadingCities" :disabled="!province || loadingCities">
                    <template #option="{ option }">
                      <div class="flex items-center gap-2 w-full">
      ...
      <template #container>
        <div class="bg-white rounded-2xl overflow-hidden">
          ...
          <div class="flex items-center justify-between p-5 border-b border-appleCore-100">
            ...
            <button type="button" class="w-7 h-7 rounded-full flex items-center justify-center
                     hover:bg-appleCore-100 text-blueberry-500" @click="closeExportDialog">
              <i class="pi pi-times text-xs" />
            </button>
          </div>
          ...
          <div class="p-5 space-y-4">
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                <!-- Current view -->
                <button type="button"
                  class="flex flex-col items-start gap-1.5 p-3 rounded-xl border-2 text-left transition-all" :class="exportScope === 'current'
                    ? 'border-apricot-400 bg-apricot-50'
                    : 'border-appleCore-100 hover:border-appleCore-300'" @click="exportScope = 'current'">
                  <div class="flex items-center gap-2">
                ...
                <!-- By Batch -->
                <button type="button"
                  class="flex flex-col items-start gap-1.5 p-3 rounded-xl border-2 text-left transition-all" :class="exportScope === 'batch'
                    ? 'border-apricot-400 bg-apricot-50'
                    : 'border-appleCore-100 hover:border-appleCore-300'" @click="exportScope = 'batch'">
                  <div class="flex items-center gap-2">
                    <div class="w-7 h-7 rounded-lg flex items-center justify-center"
                ...
                <!-- By Location -->
                <button type="button"
                  class="flex flex-col items-start gap-1.5 p-3 rounded-xl border-2 text-left transition-all" :class="exportScope === 'location'
                    ? 'border-apricot-400 bg-apricot-50'
                    : 'border-appleCore-100 hover:border-appleCore-300'" @click="exportScope = 'location'">
                  <div class="flex items-center gap-2">
                    <div class="w-7 h-7 rounded-lg flex items-center justify-center"
                ...
                <!-- By Status -->
                <button type="button"
                  class="flex flex-col items-start gap-1.5 p-3 rounded-xl border-2 text-left transition-all" :class="exportScope === 'status'
                    ? 'border-apricot-400 bg-apricot-50'
                    : 'border-appleCore-100 hover:border-appleCore-300'" @click="exportScope = 'status'">
                  <div class="flex items-center gap-2">
                    <div class="w-7 h-7 rounded-lg flex items-center justify-center"
            ...
            <div v-if="exportScope === 'batch'" class="pt-2">
              ...
              <Select v-model="exportBatchId" :options="[
                { label: 'All Batches', value: null },
                ...availableBatches.map(b => ({
                  label: `${b.name} (#${b.batch_number})`,
                  value: b.id,
                }))
              ]" option-label="label" option-value="value" placeholder="Choose a batch..." class="w-full"
                size="small" />
            ...
            <div v-if="exportScope === 'location'" class="pt-2">
              ...
              ]" option-label="label" option-value="value" placeholder="All locations..." class="w-full" size="small"
                filter show-clear />
              <p class="text-[11px] text-blueberry-400 mt-1">
            ...
            <div v-if="exportScope === 'status'" class="pt-2">
              ...
              ]" option-label="label" option-value="value" placeholder="All statuses..." class="w-full" size="small"
                show-clear />
            </div>
          ...
          <div class="flex items-center justify-end gap-2 px-5 py-4
            ...
            <ExcelExportButton :mode="exportScope === 'batch' ? 'batch'
              : exportScope === 'location' ? 'location'
                : exportScope === 'status' ? 'status'
                  : 'list'" :applicants="exportApplicants" :batch-id="exportBatchId ?? undefined" :batch-name="availableBatches.find(b => b.id === exportBatchId)?.name
                    ?? folder?.label" :location="exportLocation || folder?.label"
              :status="exportStatus || undefined" label="Download .xlsx" @exported="closeExportDialog" />
          </div>
        </div>
      </template>
    </Dialog>
