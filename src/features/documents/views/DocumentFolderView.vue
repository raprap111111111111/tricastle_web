...
import type { DocumentGroup, FolderVersion } from '../types/folders'

const props = defineProps<{ applicantId: string | number }>()
...
// ── Navigation ────────────────────────────────────────────────────────────
function goBack()                 { router.push({ name: 'documents.index' }) }
  ...
  <div class="flex flex-col gap-6 p-8 max-w-[1200px] mx-auto">
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-sm text-blueberry-500">
        <button class="hover:text-apricot-600 font-medium" @click="goBack">Documents</button>
        <span class="text-blueberry-300 mx-1.5">/</span>
        <span class="font-medium text-blueberry-700">{{ folder?.applicant_name ?? 'Folder' }}</span>
      </span>
    </div>
    ...
    <div v-else-if="error" class="text-center py-20">
      ...
      <AppButton label="Back to Documents" class="mt-4" @click="goBack" />
    </div>
        ...
        <div class="flex items-start gap-5">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <h1 class="text-2xl font-serif font-bold text-blueberry-800">{{ folder.applicant_name }}</h1>
              <span v-if="folder.has_pending" class="text-[10px] font-bold uppercase tracking-wide text-amber-700 bg-amber-50 px-2 py-1 rounded">
                Pending Review
              </span>
            </div>
            <div class="flex items-center gap-3 text-sm mt-1">
              <span class="font-mono text-apricot-600 font-semibold">{{ folder.applicant_code }}</span>
              <span v-if="folder.applicant_email" class="text-blueberry-400">· {{ folder.applicant_email }}</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3 px-5 py-4 border-b border-appleCore-100">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="font-mono text-[10px] text-apricot-600 font-semibold bg-apricot-50 px-1.5 py-0.5 rounded">{{ group.document_type_code }}</span>
                <span class="text-xs text-blueberry-400">{{ group.total_versions }} version{{ group.total_versions === 1 ? '' : 's' }}</span>
              </div>
            </div>
            <DocumentStatusBadge :status="group.latest_status" />
            <Button icon="pi pi-plus" label="New Version" text size="small" class="!text-apricot-600" @click="uploadNewVersion(group)" />
          </div>
            ...
            <div v-for="v in group.versions" :key="v.id"
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 text-[11px] text-blueberry-400 mt-0.5">
                  ...
                  <template v-if="v.expiry_date"><span>·</span><span>Expires {{ formatDate(v.expiry_date) }}</span></template>
                </div>
              </div>
              <div class="flex items-center gap-0.5">
                <Button icon="pi pi-eye" text rounded size="small" class="!text-blueberry-500 hover:!text-blue-600 hover:!bg-blue-50" v-tooltip.top="'View'" @click="goToDocument(v)" />
                <Button icon="pi pi-pencil" text rounded size="small" class="!text-blueberry-500 hover:!text-apricot-600 hover:!bg-apricot-50" v-tooltip.top="'Edit'" @click="editDocument(v)" />
              </div>
            </div>
          </div>
        </AppCard>
