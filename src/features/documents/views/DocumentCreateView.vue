...
// ✅ Show batch selector ONLY when there's no applicant context
// (Because applicant → batch is auto-derived)
const showBatchSelector = computed(() => !hasApplicantContext.value && !hasBatchContext.value)

// ✅ Show applicant selector ONLY when no applicant context
...
onMounted(async () => {
  ...
  try {
    ...
    // Load applicants — filter by batch if we have one
    if (showApplicantSelector.value) {
      const params: any = { limit: 200 }
      if (form.value.batch_id) params.batch_id = form.value.batch_id
      const { data: aRes } = await http.get('/applicants', { params })
...
// ═══════════════════════════════════════════════════════════
// When batch changes → reload applicants filtered by that batch
// ═══════════════════════════════════════════════════════════
async function onBatchChange() {
  ...
  try {
    const { data } = await http.get('/applicants', {
      params: { limit: 200, batch_id: form.value.batch_id },
    })
...
async function submit() {
  ...
  try {
    // ✅ FIXED: build a typed CreateDocumentPayload — NOT FormData
...
<template>
  <div class="flex flex-col gap-6 p-8 max-w-3xl mx-auto">
    ...
    <!-- Form -->
    <form class="bg-white rounded-2xl border border-appleCore-100 p-6 space-y-6" @submit.prevent="submit">

      <!-- ═══════ CONTEXT BADGE (when batch/applicant preselected) ═══════ -->
      <div v-if="hasApplicantContext || hasBatchContext"
      ...
      <!-- ═══════ BATCH (only when NO context) ═══════ -->
      <div v-if="showBatchSelector">
        <label class="block text-xs font-semibold text-blueberry-700 uppercase tracking-wider mb-2">
          Batch <span class="text-red-500">*</span>
        </label>
        <Select v-model="form.batch_id" :options="batches" option-label="name" option-value="id"
          placeholder="Select a batch..." class="w-full" filter :loading="loadingData" @change="onBatchChange" />
      </div>

      <!-- ═══════ APPLICANT (only when NO applicant context) ═══════ -->
      <div v-if="showApplicantSelector">
        <label class="block text-xs font-semibold text-blueberry-700 uppercase tracking-wider mb-2">
          Applicant <span class="text-red-500">*</span>
        </label>
        <Select v-model="form.applicant_id" :options="applicants" option-label="full_name" option-value="id"
          placeholder="Select an applicant..." class="w-full" filter :loading="loadingData"
      ...
      <div>
        ...
        <Select v-model="form.document_type_id" :options="documentTypes" option-label="name" option-value="id"
          placeholder="Select document type..." class="w-full" filter :loading="loadingData" />
      </div>
      ...
      <div>
        ...
        <DocumentUploadDropzone :disabled="submitting" @file="form.file = $event"
      ...
      <div>
        ...
        <Textarea v-model="form.notes" rows="3" placeholder="Optional notes about this document..." class="w-full" />
      </div>

      <!-- ═══════ ACTIONS ═══════ -->
      <div class="flex items-center justify-end gap-2 pt-4 border-t border-appleCore-100">
        <Button type="button" label="Cancel" severity="secondary" outlined :disabled="submitting" @click="cancel" />
        <Button type="submit" label="Upload Document" icon="pi pi-upload" :loading="submitting"
