<!-- src/features/users/views/UserCreateView.vue -->
...
import { ROLE_OPTIONS, type CreateUserPayload } from '../types'

const router = useRouter()
const { store, handleCreate } = useUser()
...
async function submit() {
  ...
  try {
    ...
    if (created) router.push({ name: 'users.show', params: { id: created.id } })
  } finally {
    saving.value = false
  }
}
  ...
  <div class="flex flex-col gap-8 p-8 max-w-[1000px] mx-auto">
    ...
    <header class="flex flex-col gap-1">
      <div class="flex items-center gap-2 text-xs text-blueberry-400 mb-1">
        ...
        <span class="text-blueberry-600">Create</span>
      </div>
      <h1 class="text-3xl font-serif font-semibold text-blueberry-800 tracking-tight">
        Create User
      </h1>
      <p class="text-sm text-blueberry-500">Add a new staff account to the system</p>
    </header>

    <!-- ─── Personal ───────────────────────────────── -->
    <AppCard :padding="'normal'" :shadow="'soft'">
      <template #header>
        <h2 class="text-sm font-semibold text-blueberry-800">Personal Information</h2>
      </template>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <InputText v-model="form.suffix" class="!rounded-xl !border-appleCore-200" placeholder="Jr., Sr., III" />
        </div>
      </div>
    </AppCard>
    ...
    <AppCard :padding="'normal'" :shadow="'soft'">
      <template #header>
        <h2 class="text-sm font-semibold text-blueberry-800">Contact & Login</h2>
      </template>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="flex flex-col gap-1.5">
          <label class="text-xs font-semibold text-blueberry-500 uppercase">Password *</label>
    ...
    <AppCard :padding="'normal'" :shadow="'soft'">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="flex flex-col gap-1.5">
          ...
          <InputText v-model="form.employee_code" placeholder="EMP-001"
                     class="!rounded-xl !border-appleCore-200 !font-mono" />
        </div>
        <div class="flex flex-col gap-1.5">
    ...
    <AppCard :padding="'normal'" :shadow="'soft'">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="flex flex-col gap-1.5">
          <label class="text-xs font-semibold text-blueberry-500 uppercase">Profile Picture</label>
          <FileUpload
            mode="basic"
            accept="image/*"
            ...
            @select="onFileSelect"
          />
          <p class="text-xs text-blueberry-400">Max 2MB. JPG, PNG, or WEBP.</p>
        </div>

        <div class="flex items-center gap-2 pt-6">
          <Checkbox v-model="form.is_active" :binary="true" input-id="active" />
          <label for="active" class="text-sm text-blueberry-700 cursor-pointer">
            Account is active
          </label>
        </div>
      </div>
