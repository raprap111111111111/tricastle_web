...
// 🆕 Mark returned home
function onMarkReturnedClick(deployment: Deployment) {
  selectedDeployment.value = deployment
  returnedDialog.value = true
}

// 🆕 Mark contract completed
function onMarkCompletedClick(deployment: Deployment) {
  selectedDeployment.value = deployment
...
async function onReturnedConfirm(reason: string) {
  ...
  // Use applicant_batch_id if available, fallback to id
  const batchId = (selectedDeployment.value as any).applicant_batch_id
    ?? selectedDeployment.value.id
...
const selectedCountry = computed(() => selectedDeployment.value?.deployment_country ?? undefined)
const selectedCompany = computed(() => selectedDeployment.value?.deployment_company ?? undefined)
</script>
...
<template>
  <div class="flex flex-col gap-6 p-8 max-w-[1400px] mx-auto">
    ...
    <header class="flex flex-col sm:flex-row items-start justify-between gap-4">
      <div class="flex items-start gap-3">
        ...
        <div>
          <div class="flex items-center gap-2">
            ...
            <span
              class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-green-50 text-green-700 rounded-full text-xs font-medium ring-1 ring-green-200">
      ...
      <div class="flex items-center gap-2">
        <SelectButton v-model="viewMode" :options="viewModeOptions" option-label="label" option-value="value"
          size="small">
    ...
    <!-- ─── Filters ────────────────────────────────── -->
    <DeploymentFilters :countries="store.countries" @filter="onFilter" @reset="onReset" />
    ...
    <template v-else-if="viewMode === 'table'">
      <AppCard :padding="'none'" :shadow="'soft'">
        <DeploymentTable :deployments="store.deployments" :pagination="store.pagination" :loading="store.loading"
          :submitting="store.submitting" @page-change="onPageChange" @limit-change="onLimitChange" @edit="onEdit"
          @cancel="onCancelClick" @mark-returned="onMarkReturnedClick" @mark-completed="onMarkCompletedClick" />
      </AppCard>
    </template>

    <!-- ─── TIMELINE View ─────────────────────────── -->
    <template v-else>
      <DeploymentTimeline :deployments="store.deployments" :loading="store.loading" />
    </template>
