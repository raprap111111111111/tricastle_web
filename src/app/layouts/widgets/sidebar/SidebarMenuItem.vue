<template>
  <RouterLink :to="to" custom v-slot="{ navigate }">
    <div
      ...
      :class="[isActive ? 'bg-apricot-500/20 border-apricot-500/50' : 'border-transparent hover:bg-appleCore-100']"
      @click="navigate"
    >
      <div class="flex items-center px-3.5 py-2 gap-3">
        ...
        >
          <i :class="[icon, isActive ? 'text-blueberry-800' : 'text-blueberry-500', 'text-base']" />
        </div>
        <span
...
const isActive = computed(() => {
  ...
  return routesToCheck.some((path) => current === path || current.startsWith(`${path}/`))
})
</script>